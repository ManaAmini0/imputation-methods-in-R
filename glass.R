# this code is suitable for glass

my_data <- read.csv(file.choose(),header= FALSE , select = c(2:11))
s=dim(my_data)
# 
# ################ change sex to num  
# 
...
for (i in 1:11) {
  ...
  data_imp_multi <- data.frame(data_imp_multi_all[ , 1:12], tmpdata[, 2:11]) # Combine imputed Y and X1-X4 (for convenience)
  ...
  for (j in 2:m+1) {
    t=0
    # t=sum((data_imp_multi[,j]-my_data[,i])^2 , na.rm = TRUE )
